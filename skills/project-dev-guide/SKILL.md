...
- 先定位仓库，再读代码，再改代码
- 所有命令和文件操作都基于绝对路径
- `PROJECT_PATH_ALIASES` 环境变量指向的 JSON 文件是当前机器的真实 source of truth
...
读取 `PROJECT_PATH_ALIASES` 环境变量指向的 JSON 文件：

```bash
echo $PROJECT_PATH_ALIASES
# e.g. /Users/stanley/.config/project-dev-guide/path-aliases.json
```
